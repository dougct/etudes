...
Solution:
    ...
    Induction hypothesis:
        We know how to compute the balance factors and heights for all nodes in
        binary trees that have less than n nodes.

    Induction step:
        If we consider the root, we can determine its balance factor easily by
        calculating the difference between the height of its children.
        Furthermore, we can also determine the height of the root (it's the
        maximal height of the two children plus one).
*/
...
int compute_heights_and_balance_factors(const std::unique_ptr<TreeNode>& node,
...
std::vector<int> compute_balance_factors(
...
void test_complex_tree() {
  ...
  //    / \   \
    //   1   3   7
  auto root = make_node(4, make_node(2, make_node(1), make_node(3)),
                        make_node(6, nullptr, make_node(7)));
  auto balance_factors = compute_balance_factors(root);
...
void test_deeply_left_unbalanced() {
  ...
  // 1
  auto root = make_node(
      4, make_node(3, make_node(2, make_node(1), nullptr), nullptr), nullptr);
  auto balance_factors = compute_balance_factors(root);
...
void test_deeply_right_unbalanced() {
  ...
  //      \
    //       4
  auto root = make_node(
      1, nullptr, make_node(2, nullptr, make_node(3, nullptr, make_node(4))));
  auto balance_factors = compute_balance_factors(root);
...
void test_zigzag_tree() {
  ...
  //      \
    //       4
  auto root = make_node(
      1, nullptr, make_node(3, make_node(2, nullptr, make_node(4)), nullptr));
  auto balance_factors = compute_balance_factors(root);
...
void test_single_child_nodes() {
  ...
  auto root = make_node(
      5,
      make_node(3, nullptr,
                make_node(4, make_node(2, nullptr, make_node(1)), nullptr)),
      nullptr);
  auto balance_factors = compute_balance_factors(root);
